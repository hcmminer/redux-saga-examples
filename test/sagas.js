...
test('fetchPosts Saga', (t) => {
  ...
  t.deepEqual(generator.next(mockPosts).value, put(actions.receivePosts(redditName, mockPosts)), 'must dispatch a receivePosts action with posts');
...
test('invalidateReddit Saga', (t) => {
  ...
  t.deepEqual(generator.next({ reddit: 'new_reddit_1' }).value, call(fetchPosts, 'new_reddit_1'), 'must call fetchPosts with new reddit');
...
test('nextRedditChange Saga when switching to new reddit', (t) => {
  ...
  t.deepEqual(generator.next('new_reddit').value, select(postsByRedditSelector), 'must select posts by reddit from store');
...
test('startup Saga', (t) => {
  ...
  t.deepEqual(generator.next('selected_reddit').value, fork(fetchPosts, 'selected_reddit'), 'delegates to fetchPosts to get posts');
  t.ok(generator.next().done, 'must finish');
  t.end();
});
